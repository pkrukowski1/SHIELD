...
class IntervalAlexNet(Classifier):
    """Interval AlexNet for CIFAR datasets.

    Args:
        in_shape (tuple): Input sample shape (Tensorflow format, channels last).
        num_classes (int): Number of output classes.
        verbose (bool): Print network info if True.
        arch (str): Architecture type, only 'cifar' supported.
        no_weights (bool): If True, expects weights from a hypernetwork.
        use_batch_norm (bool): Use batch normalization if True.
        bn_track_stats (bool): Track BN running stats if True.
        distill_bn_stats (bool): Add BN stats to distillation targets if True.
        init_weights (list, optional): Initial weights for the network.
    ...
    def forward(self, x: torch.Tensor, epsilon: float, weights: List[torch.Tensor], distilled_params=None, 
        ...
        """
        Compute the output midpoint and radii of this network given the input.

        Args:
            x (torch.Tensor): Input image tensor (Tensorflow format: channels last).
            epsilon (float): Radii of a hypercube around x.
            weights (list of torch.Tensor): Network weights provided by a hypernetwork.
            distilled_params (optional): Not used in this network.
            condition (optional): BatchNorm stats id or dict with batchnorm info.

        Returns:
            tuple: (mu, eps)
            mu (torch.Tensor): Output midpoint tensor.
            eps (torch.Tensor): Output radii tensor.
        """
