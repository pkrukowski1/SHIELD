...
def experiment(config: DictConfig) -> None:
    """
    Runs PGD attacks with increasing number of gradient steps and evaluates average accuracy.
    ...
    # Define range of gradient steps
    grad_steps = np.linspace(0, config.exp.max_steps, num=config.exp.no_splits)
    grad_steps = np.unique(np.round(grad_steps).astype(int))
    all_results = []

    for step in grad_steps:
        log.info(f"Running PGD attack with {step} steps")
        for model_type, current_model in [("standard", model), ("mixup", mixup_model)]:
            ...
            for task_id, dataset in enumerate(task_datasets):
                ...
                attack_cfg = OmegaConf.to_container(config.exp.attack.pgd, resolve=True)
                attack_cfg["steps"] = int(step)
                ...
                with torch.no_grad():
                    logits, _ = current_model(x=adv_input, task_id=task_id, epsilon=config.exp.attack.pgd.eps)
                    preds = logits.max(dim=1)[1]
                    acc = 100.0 * (preds == test_target).float().mean().item()

                log.info(f"[{model_type.upper()} | steps={step}] Task {task_id} Accuracy: {acc:.4f}%")
                results.append(acc)

            avg_acc = sum(results) / len(results)
            all_results.append({"steps": step, "model": model_type, "avg_accuracy": avg_acc})
            log.info(f"[{model_type.upper()} | steps={step}] Average Accuracy: {avg_acc:.4f}%")

    # Create DataFrame with all results
    df_all = pd.DataFrame(all_results)
    csv_path = os.path.join(config.exp.log_dir, "pgd_accuracy_comparison.csv")
    ...
    for model_type in df_all["model"].unique():
        ...
        plt.plot(df_model["steps"], df_model["avg_accuracy"], label=model_type.capitalize(), marker='o')

    plt.xlabel("PGD Steps", fontsize=12)
    ...
    plot_path = os.path.join(config.exp.log_dir, "pgd_accuracy_comparison.png")
